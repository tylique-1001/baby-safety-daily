...
## 覆盖类别（14类 — v6 2026-06-24 最终版）
> **全部展示在品类筛选栏，无内容品类灰色半透明不可点击**
1. 喂养器具 2. 洗护用品 3. 服饰寝具 4. 服装及布类 5. 婴幼儿食品
6. 食具类 7. 启智玩具 8. 家具类 9. 电子电器 10. 纸尿裤
11. 出行安全 12. 家居及外出必备 13. 宝宝药箱 14. 日常用品
...
## 设计规范（v6 "品类完整版" — 2026-06-24 最终版）
> **v5 已废弃**，当前使用 v6 完整品类 + 丝滑跳转版。

### 新闻来源策略
- **国内为主，海外涉华才推**：CPSC/FDA 召回默认 SKIP，仅当产品标有 China/Chinese/Made in China/天猫/京东/AmazonChina 等涉华标记时纳入
- 国内源：市场监管总局(samrdprc.org.cn/samr.gov.cn) + 中国质量新闻网(cqn.com.cn) + 央视新闻 + 财新
- 自动采集无结果时，回退到 `get_manual_today_hotspots()` 手动整理的热点数据

### 严重程度三档（取代旧的二档）
- 🔴 **紧急 (urgent)** — 致死/严重伤害/四部门联合调查/肉毒杆菌/窒息致命
- 🟡 **重要 (important)** — 召回/不合格/有毒物质检出/强制标准违反/责令下架
- 🟠 **提醒 (reminder)** — 标准更新/行业动态/声明/辟谣/调查进展/科普
...
### HTML 报告 v5 布局
- **四区展示**：🔴紧急警示 (urgent-section) → 🟡重要召回 (important-section) → 🟠提醒关注 (reminder-section) → 💡安全贴士 (tips-section)
- 每区独立 `h2.section-heading` + 锚点ID，背景色区分（红/黄/蓝/绿）
- **四栏统计卡片** (grid 4列)：紧急/重要/提醒/贴士，每卡用 `jumpToSection(id)` 实现 scrollIntoView 平滑跳转
- **品类芯片**：每条新闻卡片内 `.cat-row > .cat-chip` 显示所属品类
- **品类概览条**：统计卡片下方显示 "📂 覆盖品类：XX类 · XX · XX"
- `format_for_report()` 返回 3-tuple: (urgent, important, reminder)
- `build_v9_card()` 接受 4 个新闻参数: (urgent, important, reminder, tips)
